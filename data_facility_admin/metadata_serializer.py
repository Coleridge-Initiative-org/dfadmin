...
def __get_data_provider(metadata):
    try:
        if DATA_PROVIDER_KEY in metadata:
            data_provider_name = metadata[DATA_PROVIDER_KEY]
            metadata.pop(DATA_PROVIDER_KEY, None)
            dp = DataProvider.objects.get(name=data_provider_name)
            return dp
        else:
            return None
    except DataProvider.DoesNotExist:
        logger.info('Created Data Provider: %s' % data_provider_name)
        return DataProvider.objects.create(name=data_provider_name)
...
def load(search_gmeta, detailed_gmeta=None, given_dataset=None):
    ...
    dataset.data_provider = __get_data_provider(search_content)
